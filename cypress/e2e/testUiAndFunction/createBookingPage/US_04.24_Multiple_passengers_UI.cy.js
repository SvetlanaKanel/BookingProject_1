...
describe.skip('US_04.24 | Multiple passengers UI', () => {
    ...
    it.skip('AT_04.24.01 | Verify that Fare type dropdown menu is visible and has three options: Adult, Child and Elder', function () {
        createBookingPage.clickFareTypeDropdown();
        createBookingPage.getMainPassengerFareTypeDropdownListOptions().each(($el, i) => {
            cy.wrap($el).should('have.length', this.createBookingPage.dropdowns.fareType.faretypesNumber);
        });

class CreateBookingPage {
  ...
  getListDepartureStation = () =>
    cy.get(".select2-results .select2-results__option");
  getArrivalStationDropdown = () => cy.get("#select2-destination-container");
  getArrivalStationList = () =>
    cy.get(".select2-results .select2-results__options");
  getDepartureStationSelectionDropdown = () =>
    cy.get('select[name="departure"]');
  getArrivalStationSelectionDropdown = () =>
  ...
  getCalendarDays = () =>
    cy.get(".col-lg-12.calendar-day-selection-wrapper > .day-wrapper");
  ...
  getFridayButton = () =>
    cy.get("div .calendar-day-selection-wrapper :nth-child(5)");
  getWeekButton = () => cy.get("button.calendar-view-week");
  getSaturdayButton = () =>
    cy.get("div .calendar-day-selection-wrapper :nth-child(6)");
  ...
  getTicketsAvailableFirstTripCard = () =>
    cy.get(".trip:first-child span.availability span.num");
  getLabelDepartureOnDate = () =>
    cy.get("#label-departure-on #trips-selected-date");
  ...
  getMainPassengerNameField = () =>
    cy.get('.form-control[name="passenger-name[]"]');
  getExtraPassengerNameField = () =>
    cy.get(
      '.form-control[name="passenger-name[]"]:not(.form-control:first-child)'
    );
  ...
  getPassengersDetailsDropdown = () =>
    cy.get(".passenger-wrapper .title select.passengers-amount");
  getPassengersDetailsDropdownList = () =>
    cy.get(".passenger-wrapper .title select.passengers-amount option");
  ...
  getMainPassengerFareTypeDropdownSelect = () =>
    cy.get("div.passenger-row:not(.passenger-add) .div-fare-type select");
  getMainPassengerFareTypeDropdownList = () =>
    cy.get(
      "div.passenger-row:not(.passenger-add) .div-fare-type select option"
    );
  getAddedPassengersFareTypeDropdownLists = () =>
    cy.get("div.passenger-row.passenger-add .div-fare-type select");
  getAddedPassengerFareTypeDropdownListOptions = () =>
    cy.get('[class="select2-results__options"] li');
  getEmailInputField = () => cy.get(":nth-child(4) > .form-control");
  getAmountOfChosenPass = () =>
    cy.get(".box-default .passenger-wrapper .passenger-row");
  ...
  getMainPassengerSelectedSeatByDefault = () =>
    cy.get('div[class="col-lg-12 passenger-row"] span[class="seat-number"]');
  ...
  getExtraFareTypeDrop = () =>
    cy.get(".fare-type-box .select2-selection__rendered");

  //Seat selection
  getSeatSelectionDropdown = () =>
    cy.get(".layout-wrapper .title select.passengers-amount");
  getSeatSelectionDropdownList = () =>
    cy.get(".layout-wrapper .title select.passengers-amount option");
  getSelectedSeats = () => cy.get('table.seats tr td[class="seat selected"]');
  getRowsSeatsSeatSection = () =>
    cy.get(".seat-chart .seats tr:not(tr:first-child)");
  ...
  // Summary section
  getSeatsNumberColumnSummary = () =>
    cy.get(".total-wrapper > div.total-row :nth-child(3)");
  getRowsSummaryList = () => cy.get(".total-wrapper > div.total-row");
  getAmountOfPassengersInSummary = () =>
    cy.get(".box-default .total-wrapper  .total-row");
  ...
  selectDepartureStation(stationName) {
    ...
    this.getListDepartureStation().contains(stationName).click();
  }

  selectArrivalStation(stationName) {
    this.clickArrivalStationDropdown();
    this.getArrivalStationList().contains(stationName).click();
  }
  ...
  getRandomPassengersAmmount() {
    return this.getPassengersDetailsDropdown().then(($el) => {
      const passengersArray = $el
        ...
        .join(",")
        .split(",");
      const indexOfPassengersAmmount = Math.floor(
        Math.random() * passengersArray.length
      );
  ...
  getRandomAmountOfPassSeatSelectionDrpDwn() {
    return this.getSeatSelectionDropdownList().then(($el) => {
      let passengersArray = $el.toArray().map((el) => el.innerText);
  ...
  getRequiredDefaulDay_DDFormat() {
    let date = new Date();
    let currentTailandDate = date.toLocaleDateString("en-US", {
      day: "numeric",
  ...
  getCurrentMonthAndYear() {
    let date = new Date();
    const currentMonthAndYear = date.toLocaleDateString("en-US", {
      month: "short",
      year: "numeric",
    });
  ...
  getDefaultDayMonthYear() {
    let date = new Date();
    let currentMonthYearTailand = date.toLocaleDateString("en-US", {
      month: "short",
      year: "numeric",
      timeZone: "Asia/Bangkok",
    });
    let defaultDayMonthYear =
      this.getRequiredDefaulDay_DDFormat() + " " + currentMonthYearTailand;
